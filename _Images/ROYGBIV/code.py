...
import time
import busio
from cs import RED, ORANGE, YELLOW, GREEN, ICE_BLUE, INDIGO, VIOLET, draw_plot
import gc
...
colors = [RED, ORANGE, YELLOW, GREEN, ICE_BLUE, INDIGO, VIOLET]
...
while False:
    ...
    try:
        bg_sprite = cs_cache[c % len(colors)]
    except KeyError:
        color_bitmap = displayio.Bitmap(board.DISPLAY.width, board.DISPLAY.height, 1)
        color_palette = displayio.Palette(1)
        color_palette[0] = colors[c % len(colors)]
        
        bg_sprite = displayio.TileGrid(color_bitmap,
                                       pixel_shader=color_palette,
                                       x=0, y=0)
                                       
        cs_cache[c % len(colors)] = bg_sprite
